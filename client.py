...
SYSTEM_PROMPT = """You are a helpful islamic assistant capable of accessing external functions and engaging in casual chat.
Use the responses only if needed from these function calls to provide accurate and informative answers.
...
Always utilize tools whenever the user asked for islamic information to access real-time information when required.
...
try:
    # First, try to initialize OpenAI client with OpenAPI key
    client = AsyncOpenAI()
except:
...
async def main():
    ...
    # Configure Local-based MCP server
    server_params = StdioServerParameters(
        command="mcp",
        args=[
            "run",
            "server.py",  # Remove container after exit
        ],
        env=None,
    )
    ...
    async with MCPClient(server_params) as mcp_client:
        ...
        tools = {
            ...
            for tool in mcp_tools if tool.name != "list_tables"  # Excludes list_tables tool as it has an incorrect schema
        }
